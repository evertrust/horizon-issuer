...
func (r *IssuerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, err error) {
	...
	defer func() {
		if err != nil {
			...
			issuerutil.SetReadyCondition(issuer, &issuerStatus.Conditions, metav1.ConditionFalse, "Error", err.Error())
		}
	...
	if ready := issuerutil.GetReadyCondition(issuerStatus); ready == nil {
		...
		issuerutil.SetReadyCondition(issuer, &issuerStatus.Conditions, metav1.ConditionUnknown, "FirstSeen", "First seen")
		return ctrl.Result{}, nil
	}
	...
	issuerutil.SetReadyCondition(issuer, &issuerStatus.Conditions, metav1.ConditionTrue, "Success", "Health check succeeded")
